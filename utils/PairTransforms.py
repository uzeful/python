    ...
    def __call__(self, *pics):
        ...
        for pic in pics:
            if isinstance(pic, np.ndarray):
                ...
                img = torch.from_numpy(pic.transpose((2, 0, 1)))
                # backward compatibility
                out_pics.append(img.float().div(255))

            if accimage is not None and isinstance(pic, accimage.Image):
                nppic = np.zeros([pic.channels, pic.height, pic.width], dtype=np.float32)
                pic.copyto(nppic)
                out_pics.append(torch.from_numpy(nppic))
        ...
        for img in imgs:
            if isinstance(self.size, int):
                ...
                if w < h:
                    ow = self.size
                    oh = int(self.size * h / w)
                    out_imgs.append(img.resize((ow, oh), self.interpolation))
                else:
                    oh = self.size
                    ow = int(self.size * w / h)
                    out_imgs.append(img.resize((ow, oh), self.interpolation))
            else:
                out_imgs.append(img.resize(self.size, self.interpolation))
...
class PairPad(object):
    ...
    def __call__(self, *imgs):
        ...
        out_imgs = []
        for img in imgs:
            out_imgs.append(ImageOps.expand(img, border=self.padding, fill=self.fill))
        return out_imgs
...
class PairLambda(object):
    ...
    def __call__(self, *imgs):
        out_imgs = []
        for img in imgs:
            out_imgs.append(self.lambd(img))
        return out_imgs
...
class PairRandomCrop(object):
    ...
    def __call__(self, *imgs):
        ...
        if self.padding > 0:
            for img in imgs:
                pad_imgs.append(ImageOps.expand(img, border=self.padding, fill=0))
        else:
            pad_imgs = imgs
